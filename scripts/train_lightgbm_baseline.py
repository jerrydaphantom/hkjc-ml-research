...
try:
    ...
    raise ImportError(
        "lightgbm is required for this script. Install it in your venv with: pip install lightgbm"
    ) from exc
...
# OUTPUT FILES
# ---------------------------------
METRICS_CSV = MODELS_DIR / "baseline_metrics_lgbm_v2.csv"
PREDICTIONS_VALID_CSV = MODELS_DIR / "baseline_validation_predictions_lgbm_v2.csv"
PREDICTIONS_TEST_CSV = MODELS_DIR / "baseline_test_predictions_lgbm_v2.csv"
SPLIT_SUMMARY_CSV = MODELS_DIR / "baseline_split_summary_lgbm_v2.csv"
TRAINING_DIAGNOSTICS_JSON = MODELS_DIR / "baseline_training_diagnostics_lgbm_v2.json"

MARKET_FREE_MODEL_PATH = MODELS_DIR / "baseline_lgbm_market_free_v2.joblib"
MARKET_AWARE_MODEL_PATH = MODELS_DIR / "baseline_lgbm_market_aware_v2.joblib"

MARKET_FREE_FEATURES_JSON = MODELS_DIR / "baseline_lgbm_market_free_features_v2.json"
MARKET_AWARE_FEATURES_JSON = MODELS_DIR / "baseline_lgbm_market_aware_features_v2.json"

MARKET_FREE_IMPORTANCE_CSV = MODELS_DIR / "baseline_lgbm_market_free_feature_importance_v2.csv"
MARKET_AWARE_IMPORTANCE_CSV = MODELS_DIR / "baseline_lgbm_market_aware_feature_importance_v2.csv"
...
def prepare_categorical_train(
    ...
    for col in categorical_cols:
        ...
        for frame in [train_df, valid_df, test_df]:
            frame[col] = frame[col].where(frame[col].isin(keep_levels), "RARE").astype("category")
...
def build_prediction_frame(split_df: pd.DataFrame, pred_prob: np.ndarray, model_name: str, split_name: str) -> pd.DataFrame:
    ...
    pred_out["rank_in_race"] = pred_out.groupby(["race_date", "racecourse", "race_no"], sort=False).cumcount() + 1
    return pred_out



def build_feature_importance_df(model: lgb.LGBMClassifier, feature_cols: list[str]) -> pd.DataFrame:
    ...
    df = df.sort_values(["importance_gain", "importance_split", "feature_name"], ascending=[False, False, True])
...
def fit_and_score(
    ...
    train_df, valid_df, test_df, cat_diag = prepare_categorical_train(
        train_df=train_df,
        valid_df=valid_df,
        test_df=test_df,
    ...
    train_df, valid_df, test_df, numeric_cols_kept, num_diag = prepare_numeric_train(
        train_df=train_df,
        valid_df=valid_df,
        test_df=test_df,
    ...
    train_df, valid_df, test_df, median_map = median_impute_using_train(
        train_df=train_df,
        valid_df=valid_df,
        test_df=test_df,
    ...
    categorical_feature_final = [c for c in categorical_cols if c in feature_cols_final]
    ...
    model.fit(
        ...
        categorical_feature=categorical_feature_final,
        callbacks=[
    ...
    feature_metadata = {
        ...
        "categorical_feature_columns_used": categorical_feature_final,
    ...
    valid_pred = model.predict_proba(X_valid, raw_score=False)[:, 1]
    test_pred = model.predict_proba(X_test, raw_score=False)[:, 1]
    ...
    diagnostics = {
        ...
        "n_estimators_trained": int(getattr(model, "n_estimators_", 0) or 0),
        "median_impute_map": median_map,
        "rare_category_diagnostics": cat_diag,
        "numeric_diagnostics": num_diag,
        "top_features_by_gain": importance_df.head(20).to_dict(orient="records"),
    }
...
def main() -> None:
    ...
    print("\nTraining LightGBM market-free baseline...")
    ...
    print("Training LightGBM market-aware baseline...")
    ...
    training_diagnostics = {
        "market_free": diag_free,
        "market_aware": diag_aware,
    }
    with TRAINING_DIAGNOSTICS_JSON.open("w", encoding="utf-8") as f:
        json.dump(training_diagnostics, f, indent=2)
